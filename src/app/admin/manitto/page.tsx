...
export default async function AdminManittoPage() {
  ...
  return <AdminPageShell><AdminHeader backHref="/admin" title="마니또 관리" description="참여 기간과 공개 정책을 설정합니다." /><AdminStatGrid><AdminStatCard label="참여자" value={count ?? 0} /><AdminStatCard label="운영" value={safeSettings.is_active ? 'ON' : 'OFF'} /><AdminStatCard label="공개" value={safeSettings.reveal_enabled ? '허용' : '비공개'} /></AdminStatGrid><div className="mt-6"><ManittoSettingsForm settings={safeSettings} /></div></AdminPageShell>
}
